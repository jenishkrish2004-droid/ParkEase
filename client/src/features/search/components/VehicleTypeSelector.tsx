...
const VEHICLE_TYPES: { type: VehicleType; icon: string }[] = [
  ...
  { type: 'Commercial', icon: 'local_shipping' },
];

export function VehicleTypeSelector({ selectedVehicle, onVehicleChange }: VehicleTypeSelectorProps) {
  return (
    <div className="flex bg-secondary-100 dark:bg-[#1a1712] p-1 rounded-xl border border-secondary-200 dark:border-[#4d4635] shadow-sm max-w-max">
      {VEHICLE_TYPES.map(({ type, icon }) => (
        <button
          key={type}
          onClick={() => onVehicleChange(type)}
          className={cn(
            "relative flex items-center justify-center gap-1.5 px-4 py-1.5 rounded-lg font-semibold text-xs transition-all duration-300 z-10",
